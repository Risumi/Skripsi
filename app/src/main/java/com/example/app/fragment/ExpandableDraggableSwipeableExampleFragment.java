...
public class ExpandableDraggableSwipeableExampleFragment extends Fragment
    ...
    private void onItemViewClick(View v) {
        ...
        if (childPosition == RecyclerView.NO_POSITION) {
            ((ActivityMain) getActivity()).onGroupItemClicked(groupPosition);
        } else {
...
//            mRecyclerViewExpandableItemManager.notifyGroupItemInserted(getDataProvider().getGroupCount()-1,false);
            getDataProvider().insertChildItem(groupPosition,null);
            mRecyclerViewExpandableItemManager.notifyChildItemInserted(groupPosition,getDataProvider().getChildCount(groupPosition)-1);
        }
    }
    ...
    public AbstractExpandableDataProvider getDataProvider() {
        return ((ActivityMain) getActivity()).getDataProvider();
    }
